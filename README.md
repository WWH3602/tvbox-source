# 影视源库 - ZYFun / TVBox 配置管理

> **版本**: v2.0 | 2026-06-08
> **定位**: 统一管理 TVBox / ZYFun 影视播放器的配置文件，GitHub Pages 托管，支持自动更新。
...
| 配置文件 | 地址 | 用途 |
|:---|:---|:---|
| **ZYFun 专用** | `https://wwh3602.github.io/tvbox-source/zyfun_config.json` | ZYFun 播放器导入 |
| **TVBox 通用** | `https://wwh3602.github.io/tvbox-source/config.json` | TVBox / 影视仓等 |

---

## 内容概览

- **影视站点**: `47 个`（2026-06-08 最新）
  - 30 个 DRPY/JS 类型（爱看机器人/如意/瓜子/永乐/金牌等）
  - 17 个 T1_JSON 采集源（量子/淘片/光速/金鹰/非凡/iKun/1080zyku/U酷/无尽/红牛/最大/极速/优质/豪华/速播/魔都）
...
├── zyfun_config.json        ← ZYFun 专用（自动派生，不要手动编辑）
│
├── __tools/                 ← 脚本工具
│   ├── README.md            ← 工具说明
│   ├── auto_update.py       ← 主控脚本：拉取多仓→去重→输出测试文件
│   ├── make_zyfun_config.py ← 派生 zyfun_config.json
│   ├── add_sites.py         ← 批量添加站点
│   ├── fix_zyfun.py         ← 修复历史遗留字段
│   └── 一键更新.bat          ← Windows 一键提交推送
│
├── __sources/               ← 多仓地址簿
│   └── 多仓地址簿.md         ← 记录所有多仓来源及状态
│
├── __temp/                  ← 临时测试文件（git 忽略）
│   └── 待测试_YYYYMMDD.json  ← 自动生成的待测站点
│
└── README.md                ← 本文件
```
...
发现新多仓 / 博主推荐 →  加到 __sources/多仓地址簿.md
              ...
              └→ AI 把可用的合并进 config.json
                            ↓
            python __tools/make_zyfun_config.py --push
                            ↓
